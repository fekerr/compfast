    ...
    for path in paths:
        try:
            ...
            if interpreter in ["micropython", "circuitpython"]:
                display_name = version.split(";")[1].strip()
            ...
            paths_list.append(f"*path{len(paths_list)+1}: {path}")
...
for index, item in enumerate(data, start=1):
    print(f"{index}. {item['Interpreter']:12} {item['Version']:30} {item['Real Time (s)']:>10} *path{index}")

...
def get_version(interpreter, path):
    if interpreter in ["micropython", "circuitpython"]:
        result = subprocess.run([path], input=b'\x04', capture_output=True, text=True)
...
for index, item in enumerate(data, start=1):
    ...
    print(f"{index}. {item['Interpreter']:12} {item['Version']:30} {item['Real Time (s)']:>10} {path_display}")

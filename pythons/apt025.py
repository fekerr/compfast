...
# Function to measure runtime using loop.py
def measure_loop_runtime(path, iterations):
    result = subprocess.run(["/usr/bin/time", "-p", path, "loop.py", str(iterations)], capture_output=True, text=True)
    time_lines = result.stderr.splitlines()
    real_time = next(line for line in time_lines if line.startswith('real')).split()[1]
    return float(real_time)
...
for interpreter in interpreters:
    ...
    for path in paths:
        try:
            ...
            loop_runtime = measure_loop_runtime(path, estimated_iterations)

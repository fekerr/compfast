    ...
    for path in paths:
        try:
            ...
            if interpreter in ["micropython", "circuitpython"]:
                ...
                version_info = " ".join(version_split[1].split()[1:])
                version = f"{interpreter_name} {version_info.strip()}"
            else:
...
print("\nInterpreter                         Version Info                                           Runtime (s)")
print("===========                         ============                                           ============")
for index, item in enumerate(data, start=1):
    print(f"{index:2}. {item['Interpreter']:<30} {item['Version']:<55} {item['Real Time (s)']:>10}")
